...
class Alti_ProtectUploads_Admin {
	...
	public function enqueue_styles() {
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'assets/css/alti-protect-uploads-admin.css', array(), $this->version, 'all' );
	}
	...
	public function enqueue_scripts() {
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'assets/js/alti-protect-uploads-admin.js', array( 'jquery' ), $this->version, false );
	}
	...
	public function create_index() {
		...
		if(!file_exists( self::get_uploads_dir() .'/index.php' )) {
			...
			$indexContent = "<!-- \n Generated by " . $this->get_plugin_name() . " \n http://www.alticreation.com/en/alti-protect-uploads/ \n date:" . date('d/m/Y') . "\n--><?php // Silence is golden";
			
			if( !file_put_contents( self::get_uploads_dir().'/'.'index.php', $indexContent ) ) {
				$this->messages['file'][] = array(
					'message' => __('Impossible to create or modified the index.php file.', $this->get_plugin_name()),
					'type' => 'error',
					'id' => '1'
				);
			}
				...
				$this->messages['file'][] = array(
					'message' => __('The index.php file has been created.', $this->get_plugin_name()),
					'type' => 'updated'
				);
			}
	...
	public function create_htaccess() {
		...
		if(file_exists( self::get_uploads_dir() .'/.htaccess') && preg_match( '/(# BEGIN alti-protect-uploads Plugin)(.*?)(# END alti-protect-uploads Plugin)/is', file_get_contents( self::get_uploads_dir() .'/.htaccess' ) ) == 0) {
	...
	public function remove_index() {
		...
		if(file_exists( self::get_uploads_dir() .'/index.php' )) {
			...
			unlink( self::get_uploads_dir() .'/index.php' );
			$this->messages['file'][] = array(
				'message' => __('The index.php file has been deleted.', $this->get_plugin_name()),
				'type' => 'updated'
			);
		}
	...
	public function remove_htaccess() {
		...
		if(file_exists( self::get_uploads_dir() .'/.htaccess' )) {
			...
			$htaccessContent = preg_replace( '/(# BEGIN alti-protect-uploads Plugin)(.*?)(# END alti-protect-uploads Plugin)/is', '', $htaccessContent );
	...
	public function display_messages() {
		...
		foreach ($this->messages as $name => $messages) {
			foreach ($messages as $message) {
				require plugin_dir_path( dirname( __FILE__ ) ) . 'admin/views/includes/alti-protect-uploads-admin-message.php';
			}
		}
